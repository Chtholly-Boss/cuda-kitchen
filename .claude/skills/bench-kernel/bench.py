"""Microbenchmark utilities for CUDA kernels.

Copied from DeepGEMM's `deep_gemm/testing/bench.py` (main branch).
Canonical source: https://github.com/deepseek-ai/DeepGEMM/blob/main/deep_gemm/testing/bench.py

Two functions:

- `bench(fn, ...)` — wall-clock via torch.cuda.Event. Use for quick sanity checks
  where CPU launch overhead is negligible relative to kernel time.

- `bench_kineto(fn, kernel_names, ...)` — per-kernel timing via torch.profiler
  (Kineto). Preferred for almost all cases: isolates GPU kernel time from launch
  overhead, handles multi-kernel launches, gives per-kernel breakdowns.

See SKILL.md for when to use which, and pitfalls.
"""
